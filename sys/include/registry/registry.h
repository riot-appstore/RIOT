 ...
 * ## Architecture
 *
 * The Registry interacts with other RIOT modules via Registry Handlers, and
 * with non-volatile devices via Storage Facilities. This way the functionality
 ...
 * ### Storage Facilities
 *
 * @ref registry_store_t "Storage Facilities" (SF) implement the
 * @ref regsitry_store_itf_t "storage interface" to allow the RIOT Registry to
 * load, search and store configuration parameters. From the point of view of
 * the RIOT Registry all parameters are key/value strings, it is responsibility
 * Storage Facility to transform that to the proper format for storage.
 *
 * The @ref registry_store_itf_t "storage interface" defines:
 * - @ref registry_store_itf_t::load "load"
 * - @ref registry_store_itf_t::save "save"
 *
 ...
 * First, the RIOT registry is initialized with @ref registry_init().
 *
 * Then modules declare and register RHs for configuration groups by calling
 * @ref registry_register(). SFs are registered as sources and/or destinations
 * of configurations by calling @ref registry_src_register() and
 * @ref registry_dst_register().
 *
 * @{
 * @file
 *
